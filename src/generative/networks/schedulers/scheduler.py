...
class Scheduler(nn.Module):
    ...
    def poisson_noise(self, inp_images, scale=1.0, theta=None):
        """
        ...
        # final_arr, int_arr = np.empty(inp_images[:, 0, :, :, :].shape), np.empty(inp_images[:, 0, :, :, 0].shape)
        final_arrs = []

        for c in range(inp_images.shape[1]):
            int_arrs = []

            for d in range(inp_images.shape[4]):
                images = inp_images[:, c, :, :, d]

                images_np = images.detach().cpu().numpy()
                scale_np = scale[:, np.newaxis, np.newaxis].detach().cpu().numpy()
                ...
                # Apply Poisson noise
                noisy_sinograms = np.random.poisson(sinograms_positive * scale_np) / scale_np
                noise_sinograms = noisy_sinograms - sinograms_positive + sinograms

                # Apply inverse Radon transform to the noise
                noisy_images_np = np.array([iradon(noise_sinogram, theta=theta, filter_name=None) for noise_sinogram in noise_sinograms])
                int_arrs.append(noisy_images_np)
                
            int_arr = np.stack(int_arrs, axis=3)
            print(int_arr.shape)
            final_arrs.append(int_arr)

        final_arr = np.stack(final_arrs, axis=1)
        print(final_arr.shape)
        noisy_images = torch.from_numpy(final_arr).float().cuda()

        return noisy_images

    def add_poisson_noise(self, original_samples: torch.Tensor, noise: torch.Tensor, timesteps: torch.Tensor) -> torch.Tensor:
    # Linearly increase the strength of the Poisson noise with time step t
        # print("adding poisson noise")
        scale = (timesteps + 1) * 0.01  # base_scale is a basic scaling factor
        
        # if noise is None:
        #     print("Adding poisson_noise because noise was None")
        #     noise = self.poisson_noise(original_samples, scale)
        print("Jay's poisson_noise")
        noise = self.poisson_noise(original_samples, scale)
