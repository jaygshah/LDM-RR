...
class DiffusionInferer(Inferer):
    ...
    def __call__(
        ...
        # noisy_image = self.scheduler.add_poisson_noise(original_samples=inputs, noise=noise, timesteps=timesteps)

        # noisy_image_wo_cond = noisy_image
        ...
        # return inputs, noisy_image_wo_cond, prediction
    ...
    def sample(
        ...
        for t in progress_bar:
            ...
            image, pred_original_sample = scheduler.step(model_output, t, image)
        ...
        if save_intermediates:
            ...
            return image, pred_original_sample
...
class LatentDiffusionInferer(DiffusionInferer):
    ...
    def sample(
        ...
        outputs = sample(
            ...
            mode=mode,
            verbose=verbose,
            jay_boolean=jay_boolean,
        )

        if save_intermediates:
            latent, latent_intermediates = outputs
        else:
            latent, orig_latent = outputs

        if self.ldm_latent_shape is not None:
            latent = self.autoencoder_resizer(latent)
            # orig_latent = self.autoencoder_resizer(orig_latent)
        ...
        # orig_image = decode(orig_latent / self.scale_factor)
